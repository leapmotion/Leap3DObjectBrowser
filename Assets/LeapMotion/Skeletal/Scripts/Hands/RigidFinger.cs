...
public class RigidFinger : SkeletalFinger {
  ...
  public override void InitFinger(Transform deviceTransform) {
    base.InitFinger(deviceTransform);
  }
  ...
  public override void UpdateFinger(Transform deviceTransform) {
    for (int i = 0; i < bones.Length; ++i) {
      if (bones[i] != null) {
        // Set velocity.
        Vector3 target_bone_position = deviceTransform.TransformPoint(GetBonePosition(i));
        
        bones[i].rigidbody.velocity = (target_bone_position - bones[i].transform.position) *
                                      ((1 - easing) / Time.fixedDeltaTime);

        // Set angular velocity.
        Quaternion target_rotation = deviceTransform.rotation * GetBoneRotation(i);
